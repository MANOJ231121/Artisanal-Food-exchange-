...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Keep app screen and login screen hidden initially, show landing page
    document.getElementById('landing-screen').classList.remove('d-none');
...
function navigateToLogin(filterType) {
...
// Select profile role card
function selectRole(roleId) {
    const card = document.getElementById(`p-card-${roleId}`);
    if (card) {
        // Elastic scale-down tactile click transition
...
function performLogin(roleId) {
    currentState.currentRole = roleId;
    ...
    // Scroll to top of app workspace
    window.scrollTo({ top: 0, behavior: 'instant' });
...
function renderProducerCharts(analytics) {
    ...
    revGrad.addColorStop(0, 'rgba(31, 124, 82, 0.25)');
    revGrad.addColorStop(1, 'rgba(31, 124, 82, 0.00)');

    const barGrad = prodCtx.createLinearGradient(0, 0, 0, 250);
    barGrad.addColorStop(0, '#1f7c52');
    ...
    revenueChartInstance = new Chart(revCtx, {
        ...
        options: {
            ...
            scales: {
                y: { grid: { color: '#f1f5f9' }, ticks: { font: { family: 'Plus Jakarta Sans', size: 11 }, color: '#64748b' } },
    ...
    productChartInstance = new Chart(prodCtx, {
        ...
        options: {
            ...
            scales: {
                y: { grid: { color: '#f1f5f9' }, beginAtZero: true, ticks: { font: { family: 'Plus Jakarta Sans', size: 11 }, color: '#64748b', stepSize: 1 } },
                x: { grid: { display: false }, ticks: { font: { family: 'Plus Jakarta Sans', size: 11 }, color: '#64748b' } }
            }
        }
    });
...
function handleFileSelect(event) {
    ...
    reader.onload = function(e) {
        ...
        // Set raw base64 string to hidden form field
        ...
        document.getElementById('dropzone-area').classList.add('d-none');
    };
...
// Load users dynamically from MongoDB
async function loadUsersFromDB() {
    try {
        ...
        showToast("Error connecting to database. Using offline cache.", true);
        
        // Offline seed fallback to avoid empty screen on database down
        ROLES = {
...
// Render consumer and producer cards on login page dynamically
function renderLoginCards() {
    const consumerGrid = document.getElementById('consumer-cards-grid');
    const producerGrid = document.getElementById('producer-cards-grid');
    
    if (!consumerGrid || !producerGrid) return;
    
    consumerGrid.innerHTML = '';
    producerGrid.innerHTML = '';
    
    Object.keys(ROLES).forEach(roleId => {
        const user = ROLES[roleId];
        if (user.isProducer) {
            const cardHTML = `
                <div class="profile-card clickable" onclick="selectRole('${roleId}')" id="p-card-${roleId}">
                    <div class="profile-avatar farm"><i class="fa-solid fa-tractor text-amber"></i></div>
                    <div class="profile-details">
                        <h4>${user.name}</h4>
                        <span class="role-tag producer">Producer</span>
                        <p class="rating-txt"><i class="fa-solid fa-star text-amber"></i> ${parseFloat(user.rating).toFixed(1)} (${user.city})</p>
                    </div>
                </div>
            `;
            producerGrid.insertAdjacentHTML('beforeend', cardHTML);
        } else {
            const cardHTML = `
                <div class="profile-card clickable" onclick="selectRole('${roleId}')" id="p-card-${roleId}">
                    ...
                    <div class="profile-details">
                        <h4>${user.name}</h4>
                        <span class="role-tag consumer">Consumer</span>
                        <p class="location-txt"><i class="fa-solid fa-location-arrow"></i> ${user.city} (${parseFloat(user.lat).toFixed(1)}° N, ${parseFloat(user.lon).toFixed(1)}° E)</p>
                    </div>
                </div>
            `;
            consumerGrid.insertAdjacentHTML('beforeend', cardHTML);
        }
    });
}

// Register Profile Modal Controllers
function openAddProfileModal(roleType) {
    document.getElementById('prof-role').value = roleType;
    
    const title = document.getElementById('profile-modal-title');
    const nameLabel = document.getElementById('prof-name-label');
    const nameInput = document.getElementById('prof-name');
    
    if (roleType === 'PRODUCER') {
        title.innerText = "Add Producer Firm";
        nameLabel.innerText = "Firm Name";
        nameInput.placeholder = "e.g. Himachal Mountain Farms";
    } else {
        title.innerText = "Add Customer Profile";
        nameLabel.innerText = "Customer Name";
        nameInput.placeholder = "e.g. Manoj Sharma";
    }
    
    document.getElementById('add-profile-modal').classList.remove('d-none');
}

function closeAddProfileModal() {
    document.getElementById('add-profile-modal').classList.add('d-none');
    document.getElementById('add-profile-form').reset();
}

// Prefill Latitude and Longitude suggestions based on city input
function prefillCoordinates() {
    const cityInput = document.getElementById('prof-city').value.trim().toLowerCase();
    
    let lat = 28.6139;
    let lon = 77.2090; // Default Delhi
    
    if (cityInput.includes('dehradun')) {
        lat = 30.3165;
        lon = 78.0322;
    } else if (cityInput.includes('shimla')) {
        lat = 31.1048;
        lon = 77.1734;
    } else if (cityInput.includes('rishikesh')) {
        lat = 30.0869;
        lon = 78.2676;
    } else if (cityInput.includes('mumbai')) {
        lat = 19.0760;
        lon = 72.8777;
    } else if (cityInput.includes('bangalore')) {
        lat = 12.9716;
        lon = 77.5946;
    } else if (cityInput.includes('kullu') || cityInput.includes('manali')) {
        lat = 31.9578;
        lon = 77.1095;
    } else {
        // Pre-fill randomized coordinate in Northern India (useful for testing checkout coordinates map distance!)
        lat = (28.0 + Math.random() * 4.0).toFixed(4);
        lon = (76.5 + Math.random() * 3.5).toFixed(4);
    }
    
    document.getElementById('prof-lat').value = lat;
    document.getElementById('prof-lon').value = lon;
    showToast("Suggested coordinates loaded!");
}

// Submit profile request to MongoDB backend
async function submitNewProfile(event) {
    ...
    const lat = parseFloat(document.getElementById('prof-lat').value);
    const lon = parseFloat(document.getElementById('prof-lon').value);
    ...
    const profileData = {
        ...
        latitude: lat,
        longitude: lon,
        rating: role === 'PRODUCER' ? 5.0 : 0.0,
        bio: role === 'PRODUCER' ? `${name} is a premium local food organization.` : ""
    };
    ...
    try {
        ...
        if (res.ok) {
            showToast("Profile created successfully in database!");
            closeAddProfileModal();
            await loadUsersFromDB(); // Refresh login cards dynamically
        } else {
            showToast("Failed to create profile in database", true);
        }
    } catch (e) {
        console.error("Error creating profile:", e);
        showToast("Network error creating profile", true);
    }
}

// Smooth scrolling nav helper for landing page
